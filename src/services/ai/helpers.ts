// preparing prompts for AI and validating user access
...
//1) These functions take raw data from the database and turn it into human-readable text
function formatLeadContext(leadContext: LeadContext) {
  if (!leadContext) return "No lead context found";
  // Converts a lead object into a formatted string
  return `
...
function formatRecentActivities(recentActivities: RecentActivities) {
  ...
  // Loops through the recent activities and formats them like: [MM/DD/YYYY] ActorName — Call: Called about product
...
function formatNextReminder(nextReminder: NextReminder) {
  ...
  // Converts the next reminder into a readable string like: Follow up email (due 04/01/2026): Call lead regarding pricing
...
//2) Creates a prompt for the AI to generate a structured lead brief
...
//3) Checks if the current user can access this lead
